...
private:
    ...
    public:
        ...
        bool setProperty(const QString& name, const QVariant& value) override {
            if (name == "name") { this->name = value.toString(); return true; }
            if (name == "age") { this->age = value.toInt(); return true; }
            return false;
        }
    };
...
void TestCsDataBinding::testBindViewModelToEntity() {
    ...
    QCOMPARE(entity.name, QString("Bob"));
    QCOMPARE(entity.age, 25);
}
...
void TestCsDataBinding::testSyncToEntity() {
    ...
    QCOMPARE(entity.name, QString("Frank"));
    QCOMPARE(entity.age, 50);
}
...
void TestApplicationContext::testGetService() {
    ...
    auto svc = ctx.getService<TestCsModule::MyTestService>();
    // 可能为 nullptr（未注册），这是预期行为
}

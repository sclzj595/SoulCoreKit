...
//
// 说明:
//   - 直接使用 spdlog 的编译期格式检查
//   - 自动包含文件名/行号/函数名
//   - 适用于需要完整上下文信息的场景

#define SC_LOG_TRACE(...)    SPDLOG_LOGGER_TRACE(sc::Logger::instance().spdlogLogger(), __VA_ARGS__)
#define SC_LOG_DEBUG(...)    SPDLOG_LOGGER_DEBUG(sc::Logger::instance().spdlogLogger(), __VA_ARGS__)
#define SC_LOG_INFO(...)     SPDLOG_LOGGER_INFO(sc::Logger::instance().spdlogLogger(), __VA_ARGS__)
#define SC_LOG_WARN(...)     SPDLOG_LOGGER_WARN(sc::Logger::instance().spdlogLogger(), __VA_ARGS__)
#define SC_LOG_ERROR(...)    SPDLOG_LOGGER_ERROR(sc::Logger::instance().spdlogLogger(), __VA_ARGS__)
#define SC_LOG_CRITICAL(...) SPDLOG_LOGGER_CRITICAL(sc::Logger::instance().spdlogLogger(), __VA_ARGS__)

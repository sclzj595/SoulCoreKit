...
public:
    ...
    /// @brief 设置滑动窗口大小(毫秒,仅 SlidingWindow 算法) [v1.9.3]
    /// @param windowMs 窗口大小(默认 1000ms), 必须 > 0
    void setWindowDurationMs(int windowMs);

    /// @brief 获取滑动窗口大小(毫秒)
    int windowDurationMs() const;
...
private:
    ...
    std::atomic<int> m_windowDurationMs{1000};  ///< 滑动窗口大小(ms) [v1.9.3]
